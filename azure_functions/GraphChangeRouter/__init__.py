...
def init_servicebus_senders() -> Dict[str, ServiceBusSender]:
    ...
    servicebus_client = ServiceBusClient.from_connection_string(conn_str=NAMESPACE_CONNECTION_STR, logging_enable=True) as servicebus_client:
...
def main(
    ...
    for node in documents:  # type: ignore
        ...
        if node_label in LABELS_WITH_QUEUE:
            ...
            logging.info(f"Node {node_id} of type {node_label} does not have a queue to send to.")
